extends Node
class_name JavascriptManager

static func init(compile_files_js):
	for _file in compile_files_js:
		JavaScriptBridge.eval(loadFile(_file),true)


static func loadFile(_file: String):
	var file: FileAccess = FileAccess.open(_file, FileAccess.READ)
...
static func _js_property_call(_object,properties):
	var prop = ""
...
static func _js_method_call(_object,_method,_arg=[]):
	var args = _args_to_js(_arg)
	
	var ret = _run(_object+"['"+_method+"']("+args+")")
	return ret

static func _args_to_js(_arg):
	var args = ""
	if _arg.size()>0:
		for a in _arg:
			var tmp_new_var = a
			...
			elif typeof(tmp_new_var) == TYPE_FLOAT:
				tmp_new_var = (tmp_new_var)
				if tmp_new_var == 0:
					tmp_new_var = "0"
...
static func _run(string):
	return JavaScriptBridge.eval(string, true)
